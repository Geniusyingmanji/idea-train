...
Train Qwen3-8B + LoRA r=64 on `data/agentic_combined_v3to35/sft_demos.jsonl` (5953 demos). Hypothesis: propose rate jumps from 18% → 60%+ based on the v2-vs-v3 comparison in `experiment.md` §3.

```bash
python tools/sft_agentic_v3.py \
  --data data/agentic_combined_v3to35/sft_demos.jsonl \
...
- ✅ Combined corpus assembled: 6165 SFT + 2593 DPO + 2930 RL prompts (19 SFT rounds, 6 DPO rounds, 4 RL rounds)
