"""SFT on agentic_v3 demos (1032 rich multi-archetype trajectories).

Adapted from sft_agentic_v2.py with:
  - max_len 3072 (median demo ~2.5K tokens, max ~12K — cap at 3072)
  - LR 3e-5 (lower than v2's 5e-5 due to longer context)
  - 2 epochs
  - reads data/agentic_v3/sft_demos.jsonl
  - uses ROLLOUT_SYS_PROMPT_V2 (6-tool prompt)
"""
...
def build_dataset(sft_demos_path: Path, tokenizer, max_len: int = 3072):
    rows = []
    with sft_demos_path.open() as f:
        for line in f:
            try:
                ...
                continue
            user_msg = d.get("full_prompt") or d.get("topic", "")
            messages = [
...
def main():
    ...
    ap.add_argument("--sft-demos",
                    default="/home/azureuser/workspace-gzy/zyf/idea_train/data/agentic_v3/sft_demos.jsonl")
    ...
    print(f"  {len(rows)} usable demos (capped at max_len={args.max_len})")
    if not rows:
        return
    ...
    trainer = Trainer(
        ...
        args=TrainingArguments(
            ...
            warmup_ratio=0.05,
            logging_steps=5,
            save_strategy="no",   # avoid intermediate checkpoint save (disk tight)
            bf16=True,
            optim="adamw_torch",
